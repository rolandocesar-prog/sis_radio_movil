<!DOCTYPE html>
<html>
<head>
	<link rel="stylesheet" href="../{$direc_css}" type="text/css">
	<script type="text/javascript" src="../js/expresiones_regulares.js"> </script>
	<script type="text/javascript" src="js/validar_funcion.js"></script>
</head>
<body class="fondo">
	<center>
		<br><br>
		<h1>MODIFICAR FUNCION</h1>
		<br>
		<form action="funcion_modificar1.php" method="post" name="formu">
			<table class="listado" border="1">
				<tr>
					<th align="right">Telefonista-Operadora (*)</th><td valign="top">:</td>
					<td>
						<select name="id_telefonista_operadora">
							{foreach item=r from=$telefonista_operadora}
							<option value="{$r.id_telefonista_operadora}">{$r.nombres} - {$r.apellidos}</option>
							{/foreach}
							{foreach item=r from=$telefonistas_operadoras}
							<option value="{$r.id_telefonista_operadora}">{$r.nombres} - {$r.apellidos}</option>
							{/foreach}
						</select>
					</td>
				</tr>
				{foreach item=r from=$funcion}
				<tr>
					<th align="right">Funcion (*)</th><th>:</th>
					<td><input type="text" name="funcion" value="{$r.funcion}">  </td>
				</tr>
				<tr>
					<th align="right">Fecha Inicio (*)</th><th>:</th>
					<td><input type="date" name="fecha_inicio_fu" value="{$r.fecha_inicio_fu}">  </td>
				</tr>
				<tr>
					<th align="right">Fecha Fin (*)</th><th>:</th>
					<td><input type="date" name="fecha_fin_fu" value="{$r.fecha_fin_fu}">  </td>
				</tr>
				<tr>
					<td align="center" colspan="3">
						<input type="hidden" name="accion" value="" ="">
						<input type="hidden" name="id_funcion" value="{$r.id_funcion}">
						<input type="button" class="boton" value="Aceptar" onclick="validar();">
						<input type="button" class="boton" value="Cancelar" onclick="javascript:location.href='funciones.php';">
					</td>
				</tr>
				{/foreach}
			</table>
			<table>
				<tr>
					<td colspan="3" align="center" class="obligatorio"><b>(*)Campos Obligatorios</b></td>
				</tr>
			</table>
		</form>
	</center>
</body>
</html>